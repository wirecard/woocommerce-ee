...
class Additional_Information {
	...
	public function create_basket_from_parent_transaction( $order, $config, $payment_method, $refund_basket = [], $refunding_amount = 0 ) {
		...
		if ( $refunding_amount - $items_total > 0 ) {
			if ( $refunding_amount - $items_total - $shipping['amount']['value'] === 0 ) {
				...
				return new WP_Error( 'error', __( 'You can only refund full shipping or none :D muhahahahaha.', 'wirecard-woocommerce-extension' ) );
			}
		}
	...
	private function getTaxRateFromTaxClassDependingOnCountry( $country, $tax_classes ) {
		$wc_tax    = new WC_Tax();
		$tax_rates = $wc_tax->find_rates( array( 'country' => $country, 'tax_class' => $tax_classes ) );
