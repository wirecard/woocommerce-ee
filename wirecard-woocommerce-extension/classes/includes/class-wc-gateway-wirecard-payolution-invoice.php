...
PAYMENT_FIELD;
    ...
    public function validate_billing_shipping_address( $customer ) {
        ...
        if ( $this->get_option( 'billing_shipping_same' ) == 'yes' ) {
            ...
            foreach ( $fields as $field ) {
                $shippingAddressValue = call_user_func( array( $customer, 'get_shipping_' . $field ) );
                if ( ! empty ($shippingAddressValue ) ) {
                    $billingAddressValue  = call_user_func( array( $customer, 'get_billing_' . $field ) );
    ...
    public function validate_countries( $customer ) {
        ...
        if ( ! empty( $shippingCountry ) ) {
            if ( ! in_array( $shippingCountry, $this->get_option( 'shipping_countries' ) ) ) {
                return false;
            }
        }
        ...
        if ( ! empty( $billingCountry ) ) {
            if ( ! in_array( $billingCountry, $this->get_option('shipping_countries' ) ) ) {
                return false;
            }
        }
